namespace :karma  do
  ...
  def with_tmp_config(command, args = nil)
    Tempfile.open('karma_unit.js', "#{Rails.root}/tmp/") do |f|
      f.write unit_js(application_spec_files)
      f.flush
      system "node ./node_modules/karma/bin/karma #{command} #{f.path} #{args}"
    end
  end
  ...
  def application_spec_files
    ...
    files = Rails.application.assets.find_asset("application_spec.coffee").to_a.map {|e| e.pathname.to_s }
  end
